...
| Runtime orchestration scope | Broader orchestration pattern in guide discussions | Current baseline is sequential-by-default `/orch` with explicit single-pass parallel execution and explicit one-shot Windows `dedicated-windows`; no debate loop or persistent/attachable OS-window sessions | accepted product scope limit |
