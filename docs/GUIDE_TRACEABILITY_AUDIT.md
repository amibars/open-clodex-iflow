...
| Runtime orchestration scope | Broader orchestration pattern in guide discussions | Current baseline is sequential-by-default `/orch` with explicit single-pass parallel execution; no debate loop or dedicated OS-window lanes | accepted product scope limit |
